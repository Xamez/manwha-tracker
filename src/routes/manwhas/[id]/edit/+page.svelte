<script lang="ts">
  ...
  let title = data.manwha.title;
  let description = data.manwha.description || "";
  let note = data.manwha.note || "";
  let link = data.manwha.link || "";
  let currentChapter = data.manwha.currentChapter;
  let status = data.manwha.status;
  let rating = data.manwha.rating?.toString() || "";
  let startDate = data.manwha.startDate;
  let endDate = data.manwha.endDate || "";
  let loading = false;
  let errorMessage = "";
  let successMessage = "";
  ...
  // Auto-set end date when status changes to completed/abandoned/ended
  $: if (
    status === "completed" || status === "abandoned" || status === "ended"
  ) {
    if (!endDate) {
